...
## 0) Текущее состояние (2025-11-18)
- Общий прогресс: **≈60 % выполнено**, **≈40 % осталось** (Этапы 0–1 закрыты; на Этапе 2 готовы формы/фильтры/лента и редактирование/удаление транзакций, работают страницы счетов/категорий).
- Ниже сводная таблица статусов этапов; чек-листы ✱roadmap✱ теперь находятся сразу под этим блоком, чтобы видеть рядом, что сделано и что осталось.

| Этап | Описание | Статус | Прогресс |
| --- | --- | --- | --- |
| 0 | Скелет (Nuxt + Tailwind + PWA) | ✅ Завершён | 100 % |
| 1 | Данные и CRUD | ✅ Dexie + seed + репозитории + Pinia-сторы/селекторы + тесты | 100 % |
| 2 | Транзакции и расчёты | ▶️ В работе (создание/фильтры/лента + редактирование/удаление готовы; осталось MoneyInput/быстрые суммы и дашборд балансов) | 75 % |
| 3 | PWA и офлайн‑проверка | ⏳ Не начат | 0 % |
| 4 | JSON бэкап/импорт | ⏳ Не начат | 0 % |
| 5 | Полировка UI/A11y | ⏳ Не начат | 0 % |

- Ближайший фокус: завершить UI Этапа 2 (MoneyInput + быстрые суммы, дашборд балансов на селекторах) и начать подготовку к экспорт/импорт сценарию.
- Выявленные пробелы:
  - нет переработанного дашборда и страницы настроек (`settings.vue`), MoneyInput/быстрых сумм;
  - сценарии экспорта/импорта JSON и Zod-схема snapshot не реализованы;
  - офлайн-/PWA‑тесты и подсказки “Добавить на Домой” на iOS не проводились, TTI/мануальные проверки отсутствуют;
  - тестовый контур ограничен `useMoney` и `stores/transactions`, остальные репозитории/сторы/валидации требуют Layer 2+3 покрытий, smoke add→filter→export→import ещё не создан.

## Оставшиеся задачи до MVP (срез 2025-11-18)
- **Этап 2 — UI:** внедрить MoneyInput с маской/цифровой клавиатурой и быстрыми суммами в форму транзакции; собрать реальный дашборд (`index.vue`) с общим и поканальным балансом, пустыми состояниями и CTA.
- **Этап 3 — PWA/offline:** проверить конфигурацию `@vite-pwa/nuxt`, докрутить кеш роутов/ассетов и прогнать офлайн-сценарий на iPhone SE; добавить подсказку «Добавить на Домой» и индикатор offline.
- **Этап 4 — Бэкап:** описать и внедрить Zod-схему JSON snapshot, реализовать экспорт (скачивание файла) и импорт с предпросмотром/валидацией/подтверждением на экране настроек (`settings.vue`).
- **Этап 5 — Полировка/тесты:** добрать unit/интеграционные тесты для аккаунтов/категорий/репозиториев/валидаторов и MoneyInput; собрать smoke add→filter→export→import; покрыть UI пустыми состояниями и базовыми a11y-улучшениями.

### Параллельные дорожки (можно распределять на разные команды)
- **MoneyInput + транзакции:** `components/MoneyInput.vue` + интеграция в форму `pages/transactions.vue`, быстрые суммы. Минимальное пересечение с остальными блоками.
- **Дашборд:** `pages/index.vue` на текущих селекторах балансов, пустые состояния/CTA. Изолирован от MoneyInput.
- **PWA/offline:** правки `nuxt.config.ts` и PWA-манифеста/Service Worker, offline-индикатор/баннер «Добавить на Домой». Не трогает страницы.
- **Бэкап/импорт:** Zod-схема snapshot, экспорт/импорт и UI `pages/settings.vue`; отдельный композабл/репозиторий. Не конфликтует с PWA/дашбордом.
- **Тесты/полировка:** покрытие стор/валидаторов/MoneyInput, smoke add→filter→export→import, пустые состояния и базовая a11y. Работает поверх контрактов остальных блоков.
...
- [x] Dexie: таблицы `accounts`, `categories`, `transactions` + индексы.
- [x] CRUD аккаунтов и категорий; дефолтные категории (seed).
- [x] Pinia-сторы, селекторы балансов и покрывающие их тесты.

### Этап 2 — Транзакции и расчёты (2–4 дн.)
- [x] Форма создания транзакции с валидацией (toMinor + Zod) и сохранением через Pinia/Repositories.
- [x] Лента операций, фильтры (счёт/категория/тип/датадиапазон) и поиск по заметке.
- [x] Редактирование/удаление транзакций в ленте.
- [ ] Маска MoneyInput, быстрые суммы и дашборд с агрегатами балансов.
...
Чек-листы перенесены рядом с разделом «0) Текущее состояние», чтобы план и текущее состояние были рядом в одном блоке. Актуальная версия дорожной карты находится там.
