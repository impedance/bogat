...
- Общий прогресс: **≈50 % выполнено**, **≈50 % осталось** (Этапы 0–1 закрыты, Этап 2 в работе — страница транзакций готова: создание + фильтры/поиск + лента).
- Ниже сводная таблица статусов этапов; подробные чек-листы ✱roadmap✱ остаются в §10, чтобы не дублировать одну и ту же информацию в двух местах.
...
| 2 | Транзакции и расчёты | ▶️ В работе (создание транзакций + лента/фильтры/поиск готовы; осталось редактирование/удаление + MoneyInput/быстрые суммы) | 60 % |
| 3 | PWA и офлайн‑проверка | ⏳ Не начат | 0 % |
| 4 | JSON бэкап/импорт | ⏳ Не начат | 0 % |
| 5 | Полировка UI/A11y | ⏳ Не начат | 0 % |

- Ближайший фокус: завершить UI Этапа 2 (формы счетов/категорий, MoneyInput, улучшения фильтров/редактирования) и начать подготовку к экспорт/импорт сценарию.
- Выявленные пробелы:
  - нет страниц и компонентов из §7/§9 для аккаунтов/категорий/настроек (дашборд пустой, отсутствуют `accounts.vue`, `categories.vue`, `settings.vue`, MoneyInput и переработанный дашборд);
  - сценарии экспорта/импорта JSON, экран настроек и Zod-схема snapshot не реализованы;
  - офлайн-/PWA‑тесты и пользовательские подсказки “Добавить на Домой” на iOS не проходили, TTI/мануальные проверки отсутствуют;
...
- [ ] Редактирование/удаление транзакций, маска MoneyInput и быстрые суммы.
