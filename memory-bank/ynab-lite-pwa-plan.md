...
- **Этап 2 — UI:**
  - собрать `components/MoneyInput.vue` с маской, цифровой клавиатурой и быстрыми суммами, внедрить в форму `pages/transactions.vue`;
  - пересобрать `pages/index.vue` на балансных селекторах (общий + по счетам), вывести пустые состояния и CTA на добавление сущностей.
- **Этап 3 — PWA/offline:**
  - проверить и донастроить `@vite-pwa/nuxt` (манифест, workbox, кеширование роутов/ассетов);
  - добавить офлайн-индикатор и инструкцию «Добавить на Домой» для iOS, провести ручной офлайн-тест на iPhone SE.
- **Этап 4 — Бэкап:**
  - описать версионированную Zod-схему JSON snapshot для Dexie данных;
  - реализовать экспорт (кнопка скачивания) и импорт (предпросмотр → валидация → подтверждение) на новой странице `pages/settings.vue`.
- **Этап 5 — Полировка/тесты:**
  - расширить unit/интеграционные тесты для аккаунтов/категорий/репозиториев/валидаторов и MoneyInput;
  - собрать smoke-сценарий add→filter→export→import и добрать пустые состояния + базовые a11y улучшения.
