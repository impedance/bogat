...
## 0) Текущее состояние (2025-11-15)
- Общий прогресс: **≈35 % выполнено**, **≈65 % осталось** (Этапы 0–1 закрыты, Этап 2 ещё не стартовал).
...
| 2 | Транзакции и расчёты | ⏳ Не начат (UI/формы/фильтры отсутствуют) | 0 % |
| 3 | PWA и офлайн‑проверка | ⏳ Не начат | 0 % |
| 4 | JSON бэкап/импорт | ⏳ Не начат | 0 % |
| 5 | Полировка UI/A11y | ⏳ Не начат | 0 % |

- Ближайший фокус: реализовать UI Этапа 2 (формы счетов/категорий/транзакций, лента, фильтры, поиск) и начать подготовку к экспорт/импорт сценарию.
- Выявленные пробелы:
  - нет страниц и компонентов из §7/§9 (дашборд пустой, отсутствуют `transactions.vue`, `accounts.vue`, `categories.vue`, `settings.vue`, MoneyInput и список транзакций);
