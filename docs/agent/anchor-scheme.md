...
- Вопрос к людям/команде: `AICODE-ASK: <question?>; decision: <что нужно>; impact: <что меняется>`
...
- Закрытые вопросы `AICODE-ASK` конвертируй в `AICODE-NOTE` с `decision:` и `ref:`.
